...
在关注于低层次的细节之前,确保你使用的集合适合你。 确保你的数据结构没有非预期的渐近复杂度。各种Scala集合的复杂性描述在[这儿](http://www.scala-lang.org/docu/files/collections-api/collections_40.html)。

性能优化的第一条原则是理解你的应用为什么这么慢。不要没有数据就动手；先对你的应用做性能分析^[[Yourkit](http://yourkit.com) 是一个很好的profiler]。首先要关注的是热循环(hot loops) 和大数据结构.过度关注优化通常是浪费精力。记住Knuth(高德纳)的格言：“过早优化是万恶之源”。
...
*线程提供了一种表达并发的方式：它们给你独立的，堆共享的(heap-sharing)由操作系统调度的执行上下文。然而，在java里线程的创建是昂贵的，是一种必须托管的资源，通常借助于线程池。这给程序员造成了额外的复杂性，也造成高度的耦合：很难从对底层资源的使用中分离出应用逻辑。

这种复杂度在创建扇出型(fan-out)的服务时尤其明显： 每个输入请求导致一大批对另一层系统的请求。在这些系统中，线程池必须被托管以便根据每一层请求的比例来平衡：管理不善的线程池会渗入到另一个里(bleeds into another)。
...
Futures允许程序员用一种声明风格，可扩充的，有处理失败原则的，来表达并发计算。这些特性使我们相信它们尤其适合用于函数式编程，这也是鼓励使用的风格。

*尽量转换future，而不是自己去创建。Future的转换(transformations)确保失败会传播，可以通过信号取消，对于程序员来说不必考虑java内存模型的含义。甚至一个仔细的程序员会写出下面的代码，顺序的发出10次RPC请求打印结果：
...
*用递归术语来表达你的问题常常会是简化的，如果应用了尾递归优化(可以通过@tailrec注解检测)，编译器甚至会将你的代码转换为正常的循环。对比一个标准的命令式版本的堆排序(fix-down):
